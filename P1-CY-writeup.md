...
3. Next we are masking the image with a region of interest(ROI), since we know in which are the lanes will appear. The mounting of the camera determines the area and pattern of the ROI. That's why in a real world example the ROI should be connected to the extrinsic and intrinsic calibration of the camera. The ROI starts at the bottom of the image and ends at the skyline in the image. The output images should only remain with lines that are on the road.
...
1. This approach will not hold for complex lane marks in city scenarios since we are only able to detect on lane in driving direction.
